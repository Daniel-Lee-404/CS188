...
class SigmoidNeuron(object):
    def __init__(self, numFeatures):
        ...
        """YOUR CODE HERE"""
        self.weightVector = np.zeros(numFeatures)
    ...
    def classify(self, sample):
        ...
        """YOUR CODE HERE"""
        if self._sigmoid(sample.dot(self.weightVector)) > .5:
            return True
        else:
            return False
    ...
    def train_single(self, sample, alpha, label):
        ...
        """YOUR CODE HERE"""
        helperF = lambda w: self.loss(sample, label, w)
        grad = nu.gradient(helperF, self.weightVector)
        trainVect = [alpha*g for g in grad]
        self.weightVector -= trainVect
    ...
    def train(self, samples, alpha, labels):
        ...
        for i,s in enumerate(samples):
            self.train_single(s, alpha, labels[i])
