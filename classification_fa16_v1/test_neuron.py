...
alpha = 0.01
num_train_examples = 5000  # can increase up to 5000 if you are patient
...
nClass = neuron.SigmoidNeuron(len(featurized_training_data[0]))
for i in range(num_times_to_train):
    nClass.train(featurized_training_data[:num_train_examples], alpha, training_labels[:num_train_examples])
test_accuracy = 1 - dcu.zero_one_loss(nClass, featurized_test_data, test_labels)
training_accuracy = 1 - dcu.zero_one_loss(nClass, featurized_training_data, training_labels)

print('Final training accuracy: ' + str(training_accuracy*100) + '% correct')

print("Test accuracy: " + str(test_accuracy*100) + '% correct')
