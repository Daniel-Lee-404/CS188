...
def fillObsCPT(bayesNet, gameState):
    ...
    housePositions = [BOTTOM_LEFT_VAL, TOP_LEFT_VAL, BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL]
    "*** YOUR CODE HERE ***"
    houseIndex = 0
    for housePos in houses:
        for wall in gameState.getHouseWalls(housePos):
            certainWall = OBS_VAR_TEMPLATE % wall
            #Create the factor with the wall,
            factor = bn.Factor([certainWall], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for assignment in factor.getAllPossibleAssignmentDicts():
                probability = 0
                if assignment[FOOD_HOUSE_VAR] == housePositions[houseIndex]:
                    if assignment[certainWall] == RED_OBS_VAL:
                        probability = PROB_FOOD_RED
                    elif assignment[certainWall] == BLUE_OBS_VAL:
                        probability = 1 - PROB_FOOD_RED
                elif assignment[GHOST_HOUSE_VAR] == housePositions[houseIndex]:
                    if assignment[certainWall] == RED_OBS_VAL:
                        probability = PROB_GHOST_RED
                    elif assignment[certainWall] == BLUE_OBS_VAL:
                        probability = 1 - PROB_GHOST_RED
                else:
                    if assignment[certainWall] == NO_OBS_VAL:
                        probability = 1
                factor.setProbability(assignment, probability)
            bayesNet.setCPT(certainWall, factor)
        houseIndex += 1
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    distribution = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR,evidence, eliminationOrder)
    maxProb = 0
    maxPos = None
    for position in distribution.getAllPossibleAssignmentDicts():
        if distribution.getProbability(position) > maxProb:
            maxProb = distribution.getProbability(position)
            maxPos = position
    return maxPos
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        inferedValues = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)

        #p(foodHouse = topLeft and ghostHouse = topRight | evidence)
        evidence[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        evidence[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL
        leftProbability = inferedValues.getProbability(evidence)

        #p(foodHouse = topRight and ghostHouse = topLeft | evidence)
        evidence[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        evidence[GHOST_HOUSE_VAR] = TOP_LEFT_VAL
        rightProbability = inferedValues.getProbability(evidence)


        #Multiply by Reward Weights
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        You can use our implementation of getExplorationProbsAndOutcomes to
        ...
        "*** YOUR CODE HERE ***"
        outcomes = self.getExplorationProbsAndOutcomes(evidence)
        for outcome in outcomes:
            expectedValue += outcome[0]*max(self.computeEnterValues(outcome[1], enterEliminationOrder))
