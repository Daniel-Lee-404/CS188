...
def joinFactors(factors):
    ...
    #First Join Unconditioned with Unconditioned and Conditioned with Conditioned
    #If Unconditioned Overlaps with Conditioned, then remove from Unconditioned
    "*** YOUR CODE HERE ***"
    unconditioned = set()
    conditioned = set()
        ...
        for aDict in newFactor.getAllPossibleAssignmentDicts():
            ...
            for oldAssignment in factor.getAllPossibleAssignmentDicts():
                ...
                for key in aDict.keys():
                    if key not in oldAssignment.keys():
                        intertwined = False
                    else:
                        if oldAssignment[key] != aDict[key]:
                            intertwined = False
                if intertwined:
                    if aDict.viewitems() <= oldAssignment.viewitems():
                        probabilitySum += factor.getProbability(oldAssignment)
            newFactor.setProbability(aDict, probabilitySum)
        return newFactor
...
def normalize(factor):
    ...
    "*** YOUR CODE HERE ***"
    #check for factors with only one variable in their domain
    unconditioned = factor.unconditionedVariables()
    conditioned = factor.conditionedVariables()
    for v in factor.unconditionedVariables():
        if len(factor.variableDomainsDict()[v]) == 1:
            unconditioned.remove(v)
            conditioned.add(v)
    newF = Factor(unconditioned, conditioned, factor.variableDomainsDict())
    newAssignments = newF.getAllPossibleAssignmentDicts()

    #Sum all assignments
    sum = 0
    for row in newAssignments:
        sum+= factor.getProbability(row)
    if sum == 0:
        return None
    for row in newAssignments:
        newF.setProbability(row, factor.getProbability(row)/sum)
    return newF
